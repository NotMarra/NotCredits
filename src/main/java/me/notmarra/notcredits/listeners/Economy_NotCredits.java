...
public class Economy_NotCredits implements Economy {
   ...
   public double getBalance(OfflinePlayer player) {
      try {
         return player != null && player.getPlayer() != null ? Database.database.getCreditsByPlayerName(player.getName()) : 0.0D;
   ...
   public EconomyResponse withdrawPlayer(OfflinePlayer player, double amount) {
      if (player == null) {
         ...
         if (amount == 0.0D) {
            ...
            try {
               Database.database.setCreditsByPlayerName(player.getName(), balance);
